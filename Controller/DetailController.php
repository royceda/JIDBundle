...
//                $qry .= ' WHERE h."ID" in (select max("ID") from SCHEDULER_HISTORY where "ERROR">0)';                
    ...
    public function order_logAction()
    {
        $request = Request::createFromGlobals();
        $id = $request->get('id');
        $dhtmlx = $this->container->get('arii_core.dhtmlx');
...
where h.ID=$id";
    ...
    public function chart_order_historyAction()
    {
        $request = Request::createFromGlobals();
        $id = $request->query->get('id');
        ...
        $sql = $this->container->get('arii_core.sql');
        $data =$dhtmlx->Connector('data');
        $qry = $sql->Select(array('soh.SPOOLER_ID','soh.ORDER_ID'))
                .$sql->From(array('SCHEDULER_ORDER_HISTORY soh'))
                .$sql->Where(array('soh.HISTORY_ID'=>$id));
        ...
        $qry2 = $sql->Select(array('soh.history_id','sosh.task_id','soh.order_id','soh.start_time','soh.end_time','sosh.error'))
                .$sql->From(array('SCHEDULER_ORDER_HISTORY soh'))
                .$sql->LeftJoin('SCHEDULER_ORDER_STEP_HISTORY sosh',array('soh.history_id','sosh.history_id'))
                .$sql->Where(array('spooler_id' => $spooler_id,
                        'order_id' => $order_id))
                .$sql->OrderBy(array('start_time DESC'));
	...
	$row->set_value("duration",strtotime($end)-$start );
        ...
        $row->set_value("id", $row->get_value("history_id").'#'.$row->get_value("task_id"));
    }
